        ...
        <div className={`w-full bg-${darkMode ? 'light' : 'dark'}`}>
            <ul className={`border border-${darkMode ? "dark" : "light"} items-center text-[#a3cbe3] flex justify-content-between py-3`}>
                <h1 className="text-3xl bold mx-5">
                    <a href="/" className={`px-2 py-1 border text-[#85A9BF] border-${darkMode ? 'dark' : 'light'} bg-${dark} rounded-[10px]`}>Library</a>
                </h1>
                <div className="flex">
                    <h1 style={{ display: darkMode ? 'none' : 'flex' }} className={`text-${darkMode ? 'dark' : 'light'}`} onClick={() => setDarkMode(true)}>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                            <path strokeLinecap="round" strokeLinejoin="round" d="M21.752 15.002A9.718 9.718 0 0118 15.75c-5.385 0-9.75-4.365-9.75-9.75 0-1.33.266-2.597.748-3.752A9.753 9.753 0 003 11.25C3 16.635 7.365 21 12.75 21a9.753 9.753 0 009.002-5.998z" />
                        </svg>
                    </h1>
                    <h1 style={{ display: darkMode ? "flex" : 'none' }} className={`text-${darkMode ? 'dark' : 'light'}`} onClick={() => setDarkMode(false)}>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                ...
                <div className="flex gap-5 mx-5">
                    <li className={`cursor-pointer bg-${dark} text-xl border border-${darkMode ? 'dark' : "light"} p-1 rounded`}>
