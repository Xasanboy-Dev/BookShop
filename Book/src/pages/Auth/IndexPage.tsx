...
export default function IndexPage({ darkMode }: { darkMode: Boolean }) {
    ...
    if (localStorage.getItem("uuid")) {
        userID = JSON.parse(localStorage.getItem("uuid")!).userID
    }
    ...
    return (
        <div className={`h-${full ? "full" : "[1115px]"} py-5  bg-${darkMode ? "light" : 'dark'}`}>
            <h1 className="text-center text-4xl p-5 text-[#85A9BF] ">
                <span className={`border border-${darkMode ? 'dark' : "light"} p-3 rounded cursor-pointer bg-dark`}>Recently Added</span>
            </h1>
            <div className="px-[50px]  grid gap-5 grid-cols-4">
                {books.map((book: Book) => (
                    <div onClick={() => { }} className={`border cursor-pointer bg-dark border-${darkMode ? "dark" : "light"} rounded py-3 px-2`}>
