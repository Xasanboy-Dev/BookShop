            ...
            <div>
                <div className=" flex m-5 text-2xl">
                    ...
                    <div>
                        ...
                        <select onChange={(e) => setAuthor(e.target.value)} className={`rounded block w-[200%] bg-[#a3cbe3] text-light border border-${darkMode ? "dark" : "light"} `}>
                ...
                <div className="flex gap-5 m-5 text-2xl">
                    <div className={`border rounded border-${darkMode ? "dark" : "light"} text-[#a3cbe3] bg-${darkMode ? "light" : "dark"}`}>
                        <label htmlFor="file-upload" className="h-[150px] w-[150px] m-2 bg-dark custom-file-upload">
                            <h1 className="my-5" style={{ display: url ? "none" : 'flex' }}> Select photo</h1>
                            <img src={`${url}`} className={`h-[150px] w-[150px]`} style={{ display: url !== "" ? "flex" : "none" }} />
                        </label>
                        <input onChange={(e) => onhange(e)} accept="image/*" name="avatar" id="file-upload" type="file" />
                    </div>
                    <div>
                ...
                <div className="flex justify-content-center gap-5">
                    ...
                    <button onClick={() => createBook(title, author, +page, url, desc, +userID)} className="border text-light rounded py-1 px-3 flex  items-center bg-green-700 " > Create</button>
                </div>
            </div>
        </div >
    )
