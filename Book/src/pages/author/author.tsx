    ...
    return (
        <div className={`h-${full ? "full" : "[907px]"} text-[#85A9BF]  bg-${darkMode ? "light" : "dark"}`}>
            <ul className={`bg-${darkMode ? 'light' : 'dark'} text-${darkMode ? 'dark' : 'light'} py-5`}>
                {authors?.map(author => (
                    <li className={`flex px-5 rounded text-xl justify-content-between items-center border border-${darkMode ? "dark" : "light"} w-[80%] mx-auto flex py-3 mb-3   justify-content-center`}>
