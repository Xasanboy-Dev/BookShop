...
export async function createAuthor(req: Request, res: Response) {
    try {
        ...
        if (checkName) {
            return res
                .status(409)
                .json({ message: 'Your author is already exist!' })
        }
        const author = await postAuthor(user.id, name)
        res.status(201).json({ message: 'Created succesfully', author })
    } catch (error: any) {
        console.log(error.message)
        res.status(500).json({ message: 'Internal error' })
    }
...
export async function editAuthor(req: Request, res: Response) {
    try {
        ...
        if (!authorID) {
            return res
                .status(404)
                .json({ message: 'Your author is not exist!' })
        }
    ...
    try {
        ...
        if (!author) {
            return res.status(404).json({
                message:
                    'Your author is not exist! Please check and try again later!',
            })
        }
